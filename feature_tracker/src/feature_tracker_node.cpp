        ...
        if (SHOW_TRACK)
        {
            ...
            for (int i = 0; i < NUM_OF_CAM; i++)
            {
                ...
                for (unsigned int j = 0; j < trackerData[i].cur_pts.size(); j++)
                {
                    ...
                    // 解释：在tmp_img上画一个半径为2的圆，圆心为cur_pts[j]，颜色为(255 * (1 - len), 0, 255 * len)，说明当前点的跟踪次数越多，颜色越接近红色
...
int main(int argc, char **argv)
{
    ...
    message_filters::Synchronizer<syncPolicy> sync(syncPolicy(10), sub_image, sub_depth);   // 定义一个时间同步器，同步两个topic的消息
    sync.registerCallback(boost::bind(&img_callback, _1, _2)); // 同步器回调函数
